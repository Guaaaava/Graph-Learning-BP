    ...
    if plot:
        ...
        for i in range(true.shape[0]):
            ...
            for agent in range(num_agents):
                label_true = 'True Track' if agent == 0 else None
                label_pred = 'Estimated Track' if agent == 0 else None
                
                plt.plot(true[i, agent, 0, :], true[i, agent, 1, :], color='black', alpha=0.6, label=label_true)
                plt.scatter(true[i, agent, 0, -1], true[i, agent, 1, -1], color='black', s=20)
                
                plt.plot(pred[i, agent, 0, :], pred[i, agent, 1, :], color="#ec43cd", alpha=0.6, label=label_pred)
                plt.scatter(pred[i, agent, 0, -1], pred[i, agent, 1, -1], color='#ec43cd', s=20)
                
            plt.scatter(anchor_pos[0, :, 0], anchor_pos[0, :, 1], color='black', marker='s', s=50, label='Anchors')
            ...
            plt.legend(loc='best')
            plt.tight_layout()
            plt.savefig('Figs/test/{}_hybrid.pdf'.format(i), format='pdf', dpi=300)
